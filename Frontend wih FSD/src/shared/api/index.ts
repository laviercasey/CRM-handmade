  ...
  async (error: AxiosError) => {
    ...
    if (error.response && error.response.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const response = await axios.post<RefreshTokenResponse>(`${api.defaults.baseURL}/auth/refresh`, { 
    ...
    if (error.response) {
      ...
      if (status === 400) {
        errorMessage = (error.response.data as any).detail || 'Некорректный запрос';
      } else if (status === 403) {
